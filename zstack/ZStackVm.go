...
//创建云主机
func CreateVmInstances(vmInstance model.CreateVmInstanceRequest) mgresult.Result {
...
//从云盘创建云主机
func CreateVmInstanceFromVolume(vmInstance model.CreateVmFromVolumeRequest) mgresult.Result {
...
//查询共享磁盘所挂载的云主机
func QueryShareableVolumeVmInstanceRef() mgresult.Result {
...
func SetVmSshKey(vmInstance model.SetVmSshKeyRequest) mgresult.Result {
	//PUT zstack/v1/vm-instances/{uuid}/actions
	url := fmt.Sprintf("zstack/v1/vm-instances/%s/actions", vmInstance.SshKey)
...
func DeleteVmConsolePassword(vmInstance model.DeleteVmConsolePasswordRequest) mgresult.Result {
	...
	dataStr, err := request.DeleteWithoutAsync(url, vmInstance)
	if err != nil {
